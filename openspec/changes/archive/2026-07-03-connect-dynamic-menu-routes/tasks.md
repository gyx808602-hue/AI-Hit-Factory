...
- [x] 1.1 阅读 `openspec/config.yaml`、`openspec/project.md`、本 change 的 `proposal.md`、`design.md` 和 `specs/dynamic-menu-routes/spec.md`。
...
- [x] 2.1 修正 `src/api/system/menus/types.ts` 中动态路由返回类型，使 `children` 能兼容真实数组、缺失值和异常值。
...
- [x] 3.1 在 `src/app/router` 邻近位置新增后端 `component` 标识到前端 `RouteKey` 的白名单映射。
...
- [x] 4.1 新增 `useCurrentUserRoutes` 或等价 hook，通过 React Query 管理动态菜单接口数据。
...
- [x] 5.1 将侧边栏菜单来源从纯静态配置切换为动态转换结果，并保留静态 fallback。
...
- [x] 6.1 为动态路由转换器新增单元测试，覆盖已知组件映射、未知组件过滤、异常 children 归一化、隐藏菜单、keepAlive 映射和外链 redirect。
- [ ] 6.2 为动态菜单初始化或路由恢复补充测试，覆盖刷新受保护页面时先加载菜单再渲染目标页。
- [ ] 6.3 补充路由跳转级测试，覆盖未登录访问受保护页跳转到 `/login`、登录后按 `redirect` 回跳、登录失效事件触发后跳转到登录页。
- [ ] 6.4 运行 `npm run typecheck`，确认动态路由类型无错误。
- [ ] 6.5 运行 `npm test`，确认新增与既有测试通过。
- [ ] 6.6 运行 `npm run build`，确认生产构建通过。
