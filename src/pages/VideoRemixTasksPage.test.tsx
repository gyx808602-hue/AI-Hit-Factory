...
describe('VideoRemixTasksPage', () => {
  ...
  it('opens create modal, creates a task and navigates to detail page', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '新建视频混剪任务' }))

    expect(await screen.findByText('创建视频混剪任务')).toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: '创建任务' }))
    expect(await screen.findByText('请输入任务名称')).toBeInTheDocument()

    fireEvent.change(screen.getByPlaceholderText('例如：混剪-618大促'), {
      target: { value: 'New Remix Task' },
    })
    fireEvent.change(screen.getByPlaceholderText('可选，补充任务说明'), {
  ...
  it('does not render inline list error alert when the list request fails', async () => {
    listPageMocks.getVideoRemixTaskPage.mockRejectedValueOnce(new Error('列表请求失败'))

    renderTaskListPage()

    await waitFor(() => {
      expect(listPageMocks.getVideoRemixTaskPage).toHaveBeenCalledTimes(1)
    })

    expect(screen.queryByText('任务列表加载失败')).not.toBeInTheDocument()
    expect(screen.queryByText('列表请求失败')).not.toBeInTheDocument()
  })

  it('does not render inline action error alert when deleting a task fails', async () => {
    listPageMocks.deleteVideoRemixTask.mockRejectedValueOnce(new Error('删除失败'))
