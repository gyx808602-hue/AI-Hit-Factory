...
import {
  ...
  type DigitalHumanVideoListText,
  type DigitalHumanVideoStatusFilterValue,
} from '../../features/digital-human/video/components/listComponents'
...
const listText: DigitalHumanVideoListText = {
  totalLabel: '任务总数',
  processingLabel: '处理中',
  successLabel: '已完成',
  failedLabel: '生成失败',
  ...
  allFilterLabel: '全部',
  processingFilterLabel: '生成中',
  successFilterLabel: '已完成',
  failedFilterLabel: '生成失败',
  loadingText: '数字人视频任务加载中...',
  emptyText: '暂无数字人视频任务',
  progressLabel: '进度',
  personIdLabel: '数字人 ID',
  durationLabel: '时长',
  secondsSuffix: '秒',
  detailButtonText: '查看详情',
  refreshButtonText: '刷新状态',
  deleteButtonText: '删除任务',
  detailAriaPrefix: '查看详情',
  refreshAriaPrefix: '刷新状态',
  deleteAriaPrefix: '删除任务',
}

type StatusFilterValue = DigitalHumanVideoStatusFilterValue
...
export function DigitalHumanVideoTasksPage() {
  ...
  const metrics = useMemo(() => {
    return buildDigitalHumanVideoMetrics(tasks, total)
  }, [tasks, total])
  ...
  return (
    ...
    >
      <DigitalHumanVideoMetricsGrid metrics={metrics} text={listText} />
      ...
      <DigitalHumanVideoTaskListSection
        ...
        tasks={tasks}
        total={total}
        currentPage={taskPageQuery.data?.pageNum ?? pageNum}
        pageSize={taskPageQuery.data?.pageSize ?? pageSize}
        text={listText}
        onViewDetail={(task) => navigate('/digital-humans/videos/' + task.id)}
        onRefresh={(task) => refreshMutation.mutate(task.id)}
