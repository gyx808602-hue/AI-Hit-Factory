...
function previewRect(values: DigitalHumanVideoFormValues) {
  return {
    ...
    width:
      (values.personWidth / Math.max(values.screenWidth, 1)) * PREVIEW_WIDTH,
    height:
      (values.personHeight / Math.max(values.screenHeight, 1)) * PREVIEW_HEIGHT,
  }
}
          ...
          <div>
            ...
            <div className="space-y-3 rounded-xl border border-[var(--line-subtle)] p-4">
              ...
              {false && (
                ...
                onChange={(event) =>
                  onChange({ ...values, bgColor: event.target.value })
                }
                />
              )}
              <ColorPicker
