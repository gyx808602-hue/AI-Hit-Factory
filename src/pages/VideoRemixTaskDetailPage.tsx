...
import { Alert, Button, Form, Input, InputNumber, Select, Space, Spin } from "antd";
import { ArrowLeft, AudioLines, CheckCircle2, FileImage, RefreshCw, UploadCloud, Video } from "lucide-react";
...
export function VideoRemixTaskDetailPage() {
  ...
  const actionMutation = useMutation({
    mutationFn: async (action: "check-prompt" | "generate-prompt" | "generate-video" | "refresh") => {
      if (!taskId) {
  ...
  async function handleUpload(
    ...
    try {
      ...
      const nextValue =
        mode === "append" && currentValue ? `${String(currentValue).trim()}\n${result.url}` : result.url;
  ...
  const characterImageUrls = useMemo(() => splitAssetUrls(characterImageUrlsText), [characterImageUrlsText]);
  ...
  return (
    ...
    >
      ...
      {actionSuccess ? (
        <Alert className="mb-4" type="success" showIcon message={actionSuccess} />
      ) : null}

      <Form<VideoRemixTaskFormValues> form={form} layout="vertical" onFinish={(values) => saveMutation.mutate(values)}>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <div className="space-y-6">
              ...
              <section data-testid="video-remix-direction-section">
                ...
                <Form.Item name="direction" label="复刻方向">
                  <Input.TextArea rows={3} placeholder="保留哪些结构、替换什么内容、输出什么改编风格" />
                </Form.Item>
              </section>
            </div>
            ...
            <section data-testid="video-remix-assets-section">
              ...
              <input
                ...
                onChange={(event) => void handleUpload(event, "referenceVideoUrl", "replace", uploadVideo)}
              />

              {referenceVideoUrl ? (
                <div className="mb-4 rounded-xl border border-[var(--line-subtle)] bg-[var(--muted-bg)] p-3">
                  <div className="mb-2 text-[12px] text-[var(--text-muted)]">参考视频预览</div>
                  <video
              ...
              <Form.Item name="videoMetaSummary" label="视频摘要">
                <Input.TextArea rows={4} placeholder="提炼视频节奏、镜头和文案结构摘要（可选）" />
              </Form.Item>

              <Form.Item name="productImageUrlsText" label="商品图">
                <div className="space-y-2">
                  <Button icon={<FileImage size={14} />} onClick={() => productImageInputRef.current?.click()}>
                    选择文件
                  </Button>
                  <Input.TextArea rows={3} placeholder="粘贴 URL 后按回车添加，或通过上传补充" />
                  <div className="text-[12px] text-[var(--text-muted)]">至少上传 1 张商品图</div>
                </div>
              </Form.Item>
              <input
                ...
                onChange={(event) => void handleUpload(event, "productImageUrlsText", "append", uploadImage)}
              />

              {productImageUrls.length > 0 ? (
                <div className="mb-4 rounded-xl border border-[var(--line-subtle)] bg-[var(--muted-bg)] p-3">
                  <div className="mb-2 text-[12px] text-[var(--text-muted)]">商品图预览</div>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {productImageUrls.map((url) => (
                      <a key={url} href={url} target="_blank" rel="noreferrer" className="block overflow-hidden rounded-lg border border-[var(--line-subtle)] bg-white">
                        <img
              ...
              <Form.Item name="characterImageUrlsText" label="人物图">
                <div className="space-y-2">
                  <Button icon={<UploadCloud size={14} />} onClick={() => characterImageInputRef.current?.click()}>
                    选择文件
                  </Button>
                  <Input.TextArea rows={3} placeholder="可选，粘贴 URL 或上传人物素材图" />
                  <div className="text-[12px] text-[var(--text-muted)]">可选，不上传则使用原视频人物</div>
                </div>
              </Form.Item>
              <input
                ...
                className="hidden"
                accept="image/png,image/jpeg,image/webp"
                onChange={(event) => void handleUpload(event, "characterImageUrlsText", "append", uploadImage)}
              />

              {characterImageUrls.length > 0 ? (
                <div className="mb-4 rounded-xl border border-[var(--line-subtle)] bg-[var(--muted-bg)] p-3">
                  <div className="mb-2 text-[12px] text-[var(--text-muted)]">人物图预览</div>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              ...
              <input
                ...
                className="hidden"
                accept="audio/*"
                onChange={(event) => void handleUpload(event, "audioUrl", "replace", uploadAudio)}
              />
            </section>
          </div>

          <section data-testid="video-remix-prompt-section" className="mt-8 border-t border-[var(--line-subtle)] pt-6">
            <SectionTitle title="Prompt 与结果" />
            <div className="mb-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
              <div>
                <div className="mb-2 text-[12px] text-[var(--text-muted)]">生成的 Prompt</div>
              ...
              <div className="space-y-2 rounded-lg border border-[var(--line-subtle)] bg-[var(--muted-bg)] p-3 text-[12px] text-[var(--text-secondary)]">
                ...
                <div>视频模型：{task.videoModel || task.targetVideoModel || "-"}</div>
            ...
            {statusMeta.resultState === "success" ? (
              <div className="mb-4 space-y-2 rounded-lg border border-[var(--line-subtle)] bg-[var(--muted-bg)] p-4 text-[12px] text-[var(--text-secondary)]">
                {task.videoUrl ? (
                  <video className="mb-3 w-full rounded-lg border border-[var(--line-subtle)]" controls src={task.videoUrl} />
                ) : null}
            ...
            <div className="flex flex-wrap justify-between gap-3 border-t border-[var(--line-subtle)] pt-4">
              ...
              <div className="flex gap-2">
                <Button onClick={() => navigate("/viral-remix/tasks")}>取消</Button>
